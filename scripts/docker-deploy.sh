...
readonly -A COMPOSE_TEMPLATE=([name]="compose.yml" [location]="https://raw.githubusercontent.com/RocketChat/Docker.Official.Image/master/compose.yml")
...
readonly -A COMPOSE_TRAEFIK_TEMPLATE=([name]="traefik.yml" [location]="https://raw.githubusercontent.com/RocketChat/Docker.Official.Image/master/traefik.yml")
...
main() {
  ...
  post_install_docker
  download_files
  if ! ((NO_AUTH)); then generate_passwords; fi
  deploy
}
